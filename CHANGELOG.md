...
- BREAKING: `apm install` now exits non-zero when target exclusion leaves an
  ...
  `apm install <owner>/<repo>/skills/<skill> --target <target>`, or select
  `--target copilot` for native registration; mixed installs still exit 0 when
  at least one package deploys. (closes #2796) (#2806)
