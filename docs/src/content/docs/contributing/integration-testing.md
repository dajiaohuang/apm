...
- **Generated lifecycle model**: a pure intent model and one observed driver power both Hypothesis search and deterministic CLI replay. `tests/fixtures/lifecycle_transition_corpus.json` contains a legal covering tour and explicit replay sequences; the tour must fire every transition and evaluate every fixture law. Failures include the case, sequence, and failed law. Every action captures complete project and user roots independently of the product ledger. Fast law-corruption and corpus contracts run in required smoke; real CLI replay/search stays in the merge-group family.
- **Routing-cell coverage**: `test_primitive_target_covering_array.py` derives static catalog rows from `KNOWN_TARGETS` (`tests/utils/lifecycle_interactions.py`) and exercises every valid primitive/target/project-or-user-scope cell, plus named target-set transitions in both scopes and two dynamic-target refusals. Every command observes the complete project and user roots. Allowed writes come from fixture sources and reviewed target layouts, not the resulting deployment ledger; unrelated files and shared configuration members must survive.
- **Source-interaction coverage**: the catalog cells are combined with direct/transitive packages, Git/local sources, compatible ref/cache states, clean/tampered content, and reinstall/audit/update actions. A pair or named triple earns credit only when at least one executed case supplies all its required law and action witnesses. CI combines the four shards' JUnit evidence and requires every scheduled case to supply evidence; the named idempotency gap remains visible rather than earning successful coverage credit. Reports link newly tracked product bugs to their regression cases, but filing a bug grants no coverage or CI exemption. `tests/quality/test_lifecycle_interaction_coverage.py` guards generation and accounting without CLI calls.
- **Command intent**: a no-change update is not a promise to repair tampered native files. The campaign observes update separately from an explicit repair install, then checks convergence on a subsequent install. Stable-target fixtures declare their target set in the manifest; experimental-target audit limitations remain visible rather than being skipped. Target contraction uses install in both scopes; the companion prune step runs only at project scope because `apm prune` has no `--global` option.
- **Runtime and scheduling**: `ApmLifecycleRunner.scenario()` applies one deadline across interleaved commands and fixture actions. Row counts are not runtime guarantees; hosted merge-group duration remains authoritative. The routing/interaction rows run in the unfiltered integration and broad release suites, not the required PR smoke set or the focused Intel release selection.
- **Out of scope for this campaign**: marketplace/OCI sources, virtual packages, MCP/LSP-only dependencies, and dynamic-target success paths. Other suites are not treated as witnesses here. This does not claim the full source-input space is exhausted or a fixed wall-clock bound is proven.
- **Run source-interaction coverage locally**:
  ```bash
  APM_E2E_TESTS=1 uv run --frozen --extra dev pytest tests/integration/test_primitive_target_covering_array.py \
    --junitxml=lifecycle-junit.xml -o junit_family=legacy
  uv run --frozen --extra dev python -m tests.utils.lifecycle_interaction_report \
    --junit lifecycle-junit.xml --revision "$(git rev-parse HEAD)" \
    --output lifecycle-interactions.json --require-complete
  ```
...
- **Known gap (Copilot user instructions)**: a repeated global Copilot instruction install can add a duplicate block under the lock identity before a third install converges byte-stable. A passing safety characterization bounds the changes and requires convergence; a separate narrow strict expected-failure test expresses the desired second-pass idempotency. The known-gap row receives no successful interaction credit. Unifying remote and lock identities is a separate production fix. Both known gaps are recorded in `tests/fixtures/lifecycle_bug_ledger.json`.
